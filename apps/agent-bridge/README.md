...
Install dependencies once with `pnpm install` at the workspace root, then from `apps/agent-bridge` in PowerShell:
...
1. Build/reload the extension (`pnpm --filter pointback-browser-extension build` from the workspace root) and refresh the inspected page.
